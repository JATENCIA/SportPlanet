...
export default function SellProduct(){
    ...
    const validatorPage1 = (post) => {
        ...
        if(post.name.length < 3) errors.name = "Product name must be higher than 3 characters!";
        if(post.name.length > 50) errors.name = "Product name must be less than 50 characters!";
        if(!post.name) errors.name = "Product name is required!";
        if(post.description.length > 500) errors.description = "Description must be less than 500 characters";
        if(post.description.length < 30) errors.description = "Description must be higher than 30 characters";
        if(!post.description) errors.description = "Product description is required!"
    ...
    const clickPage2_next = (event) => {
        ...
        if(Object.keys(e).length === 0){
            if (post.category === "tshirts" || post.category === "pants") {
                const size = Object.entries(sizes).map(([size,amount]) => ({ [size]: amount }));
                const secondSize = Object.entries(secondSizes).map(([size,amount]) => ({ [size]: amount }));
                ...
                const shoesSizes = Object.entries(shoes).map(([size, amount]) => ({ [size]: amount }));
                const secondShoesSizes = Object.entries(secondShoes).map(([size,amount]) => ({[size]: amount}));
                const thirdShoesSizes = Object.entries(thirdShoes).map(([size,amount]) => ({[size]: amount}));
                setPost({
                    ...post,
                    productConditionals:[...post.productConditionals,
                ...
                setPost({
                    ...
                    productConditionals:[...post.productConditionals,
                        ...
                        {color:secondColor,image:secondImage,size:[{"amount":secondAmount}]},
                        {color:thirdColor, image: thirdImage, size:[{"amount":thirdAmount}]}
                        ]
                });
            };
            setErrors2(e);
    ...
    const dropHandler = (images) => {
        const uploader = images.map(async (image) => {
            ...
            setLoading("true")
            const post = await axios.post("https://api.cloudinary.com/v1_1/santiagosdeb/image/upload", formData, {
    ...
    return (
        ...
        <form onSubmit={submitHandler}>
            ...
            <div className="mt-5">
            <label className="block text-gray-800 text-xl font-bold">Category</label>
            <select onChange={categoryChangeHandler} name='category' value={post.category} className={style.input}>
            ...
            <div className="mt-5">
                ...
                <label className="block text-gray-800 text-xl font-bold">Product Color</label>

                <select onChange={colorHandler} className={style.input} value={color}>
                <option value="" className="bg-white text-black-400">Select a color</option>
                <option value="purple" className={style.coloresPurple}>Purple</option>
                <option value="blue" className={style.coloresBlue}>Blue</option>
                <option value="lightBlue" className={style.coloresLightBlue}>Light Blue</option>
                <option value="green" className={style.coloresGreen}>Green</option>
                <option value="yellow" className={style.coloresYellow}>Yellow</option>
                <option value="orange" className={style.coloresOrange}>Orange</option>
                <option value="red" className={style.coloresRed}>Red</option>
                <option value="white" className={style.coloresWhite}>White</option>
                <option value="gray" className={style.coloresGray}>Gray</option>
            ...
            <Dropzone className={style.dropzone} onChange={(e)=>setImage(e.target.value)} onDrop={dropHandler}>
                {({getRootProps, getInputProps}) => (
                    <section className={style.dropzone}>
                        <div {...getRootProps()}>
                            <input {...getInputProps()}/>
                            <span><i className="fa-regular fa-folder"></i></span>
                            <p>Drop your images here, or click to select them.</p>
                        </div>
                    </section>
                )}
            </Dropzone>
            ...
            <input type='number' value={sizes.M !== 0 ? sizes.M : ""} onChange={sizesChangeHandler} name="M" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={sizes.L !== 0 ? sizes.L : ""} onChange={sizesChangeHandler} name="L" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={sizes.XL !== 0 ? sizes.XL : ""} onChange={sizesChangeHandler} name="XL" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={sizes.XXL !== 0 ? sizes.XXL : ""} onChange={sizesChangeHandler} name="XXL" className={style.sizeNumber} min='0'/>
            </div>
            ...
            :
            <div className="mt-5">
            <label className="block text-gray-800 text-xl font-bold">Amount</label>
            <input type='number' className={style.input} value={amount !== 0 ? amount : ""} onChange={amountHandler} min='0'/>
            </div>
            }
            </>
        )}
            ...
            <>
            <div className="mt-5">
                <h1 className="text-3xl font-bold text-gray-700 mb-5 text-center">Second Color</h1>

                <label className="block text-gray-800 text-xl font-bold">Product Color</label>
                <select onChange={secondColorHandler} className={style.input} value={secondColor}>
                <option value="" className="bg-white text-black-400">Select a color</option>
                <option value="purple" className={style.coloresPurple}>Purple</option>
                <option value="blue" className={style.coloresBlue}>Blue</option>
                <option value="lightBlue" className={style.coloresLightBlue}>Light Blue</option>
            ...
            <input type='number' value={secondSizes.S !== 0 ? secondSizes.S : ""} onChange={secondSizesChangeHandler} name="S" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={secondSizes.M !== 0 ? secondSizes.M : ""} onChange={secondSizesChangeHandler} name="M" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={secondSizes.L !== 0 ? secondSizes.L : ""} onChange={secondSizesChangeHandler} name="L" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={secondSizes.XL !== 0 ? secondSizes.XL : ""} onChange={secondSizesChangeHandler} name="XL" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={secondSizes.XXL !== 0 ? secondSizes.XXL : ""} onChange={secondSizesChangeHandler} name="XXL" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <div className="mt-5">
            <label className="block text-gray-800 text-xl font-bold">Amount</label>
            <input type='number' className={style.input} value={secondAmount !== 0 ? secondAmount : ""} onChange={secondAmountHandler} min='0'/>
            </div>
            }
            ...
            :<button onClick={(e) => {e.preventDefault();setSecondColorFlag(true)}} className="border border-dashed border-gray-500 py-2 px-4 rounded mx-auto block mt-8"> + Add new color</button>
            }
            ...
            <>
            <div className="mt-5">
                <h1 className="text-3xl font-bold text-gray-700 mb-5 text-center">Third Color</h1>

                <label className="block text-gray-800 text-xl font-bold">Product Color</label>
                <select onChange={thirdColorHandler} className={style.input} value={thirdColor}>
                <option value="" className="bg-white text-black-400">Select a color</option>
                <option value="purple" className={style.coloresPurple}>Purple</option>
                <option value="blue" className={style.coloresBlue}>Blue</option>
                <option value="lightBlue" className={style.coloresLightBlue}>Light Blue</option>
            ...
            <input type='number' value={thirdSizes.S !== 0 ? thirdSizes.S : ""} onChange={thirdSizesChangeHandler} name="S" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={thirdSizes.M !== 0 ? thirdSizes.M : ""} onChange={thirdSizesChangeHandler} name="M" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={thirdSizes.L !== 0 ? thirdSizes.L : ""} onChange={thirdSizesChangeHandler} name="L" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={thirdSizes.XL !== 0 ? thirdSizes.XL : ""} onChange={thirdSizesChangeHandler} name="XL" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <input type='number' value={thirdSizes.XXL !== 0 ? thirdSizes.XXL : ""} onChange={thirdSizesChangeHandler} name="XXL" className={style.sizeNumber} min='0'/>
            </div>
            ...
            <div className="mt-5">
            <label className="block text-gray-800 text-xl font-bold">Amount</label>
            <input type='number' className={style.input} value={thirdAmount !== 0 ? thirdAmount : ""} onChange={thirdAmountHandler} min='0'/>
            </div>
            }
            {/* <p className={style.errors}>{errors.size}</p> */}
            </>
